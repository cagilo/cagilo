...
class CagiloServiceProvider extends ServiceProvider
{
    ...
    public function boot(IconFinder $iconFinder): void
    {
        ...
        collect(config('cagilo.components', []))
            ->each(fn ($class, $alias) => Blade::component($alias, $class));

        collect(config('cagilo.icons', []))
            ->each(fn ($path, $key) => $iconFinder->registerIconDirectory($key, $path));
    }
